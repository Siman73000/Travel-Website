...
function hintFor(name: string) {
  ...
  if (ext === ".heic" || ext === ".heif") {
    return "This photo format may not display on all devices. If it looks blank, try saving it as JPG or PNG.";
  }
  return "This photo couldn’t be loaded right now.";
}
...
export function Gallery({
  ...
  const shownCount = Math.max(0, items.length - failedCount);
  ...
  if (items.length === 0) {
    return (
      <div className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-zinc-100/85 soft-shadow">
        ...
        <p className="mt-2 text-zinc-100/75">No photos have been added for this place yet.</p>
      </div>
    );
  }
    ...
    <div className="mt-8">
      <div className="mb-3 flex items-end justify-between gap-3">
        ...
        <div className="text-xs text-zinc-100/60">
          Showing {shownCount}/{items.length}
        </div>
      </div>

      {/* Masonry-like layout using CSS columns (static-export friendly) */}
      <div className="columns-1 gap-3 sm:columns-2 lg:columns-3">
        {items.map((file) => {
          const src = withBasePath(`/photos/${slug}/${file}`);
          ...
          return (
            <button
              ...
              aria-label={isFailed ? `Failed photo ${file}` : `Open photo ${file}`}
            >
              {isFailed ? (
                <div className="p-3">
                  <div className="text-xs font-semibold text-zinc-100">Couldn’t load</div>
                  <div className="mt-1 break-all text-xs text-zinc-100/70">{file}</div>
                  <div className="mt-2 text-[11px] text-zinc-100/60">{hintFor(file)}</div>
                </div>
              ) : (
          ...
          >
            ...
            >
              ...
              <div className="max-h-[78vh] overflow-auto p-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={active} alt="Selected photo" className="h-auto w-full rounded-2xl object-contain" />
              </div>
            </motion.div>
          </motion.div>
        )}
